function Show-McConnellHogCutOrder {
    <#
.SYNOPSIS
Render a McConnell hog cut order input UI (UniversalDashboard components) and submit to Save-DonBeefCutOrder.
#>
    [CmdletBinding()]
    param(
        [Parameter()]
...
"@
    ...
    New-UDStyle -Style $style -Content {
        ...
        New-UDElement -Tag 'div' -ClassName 'butcher-stepper-wrapper' -Content {
            ...
            New-UDCard -ClassName 'butcher-stepper-card psu-stepper-card' -Title "McConnell's Farm Market Hog Cut Order" -Content {

                # Dynamically build steps from constructor parameters
                $steps = Build-CutOrderStepper -ConstructorName 'New-McConnellHogCutOrder'
